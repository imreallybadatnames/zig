...
pub fn connectTcp(client: *Client, host: []const u8, port: u16, protocol: Connection.Protocol) ConnectTcpError!*Connection {
    ...
    const node = try client.allocator.create(ConnectionPool.Node);
    ...
    errdefer client.allocator.free(node.data.host);
    ...
    switch (protocol) {
        .tls => {
            ...
            node.data.stream_writer = .{
                .unbuffered_writer = stream.writer(),
                .buffer = &.{},
            };

            tls_client.* = std.crypto.tls.Client.init(stream, &node.data.stream_writer, .{
                .host = .{ .explicit = host },
